...
export default function StoreDetailPage() {
  ...
  const premium = isStorePremium(displayData);

  return (
    <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6">
            <Avatar className="h-24 w-24 rounded-lg border">
                <AvatarImage src={`https://picsum.photos/seed/${displayData.id}/200`} data-ai-hint="store logo" />
            ...
            <div className="mt-4 sm:mt-0">
                <div className='flex items-center gap-3'>
                    <h1 className="text-3xl font-bold font-headline">{displayData.name}</h1>
                    <Badge variant={displayData.isActive ? "default" : "destructive"} className={displayData.isActive ? "bg-emerald-500/10 text-emerald-700 border-emerald-500/20 hover:bg-emerald-500/20" : ""}>
                        {displayData.isActive ? 'Aktif' : 'Tidak Aktif'}
                    </Badge>
                    {premium && <Badge className="bg-amber-500/10 text-amber-700 border-amber-500/20 hover:bg-amber-500/20">Premium</Badge>}
                </div>
                <p className="text-muted-foreground mt-1">{displayData.description || 'Tidak ada deskripsi toko.'}</p>
            </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="md:col-span-2">
            <CardHeader>
                <CardTitle className="font-headline text-lg flex items-center"><Building className="mr-2" /> Informasi Toko & Pemilik</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-sm">
                <div className="flex items-start">
                    <MapPin className="mr-3 mt-1 h-4 w-4 text-muted-foreground flex-shrink-0" />
                    <span>{displayData.address || 'Alamat tidak tersedia di data pengguna'}</span>
                </div>
                <div className="flex items-center">
                    <Wallet className="mr-3 h-4 w-4 text-muted-foreground" />
                    <span>Saldo Token: <strong>{formatNumber(displayData.pradanaTokenBalance)}</strong></span>
                </div>
                {displayData.premiumCatalogSubscriptionExpiry && (
                    <div className="flex items-center">
                        <CalendarClock className="mr-3 h-4 w-4 text-muted-foreground" />
                        <span>Premium hingga: <strong>{format((displayData.premiumCatalogSubscriptionExpiry as Timestamp).toDate(), 'd MMMM yyyy')}</strong></span>
                    </div>
                )}
                 <div className="flex items-start">
                    <User className="mr-3 mt-1 h-4 w-4 text-muted-foreground flex-shrink-0" />
                    <span>Pemilik/Kontak Utama: <strong>{displayData.ownerName || 'Nama pemilik tidak tersedia'}</strong></span>
                </div>
                <div className="flex items-center">
                    <Mail className="mr-3 h-4 w-4 text-muted-foreground" />
                    <span>{displayData.contactEmail || 'Email tidak tersedia'}</span>
                </div>
                <div className="flex items-center">
                    <Phone className="mr-3 h-4 w-4 text-muted-foreground" />
                    <span>{displayData.contactPhone || 'Telepon tidak tersedia'}</span>
                </div>
            </CardContent>
        </Card>
