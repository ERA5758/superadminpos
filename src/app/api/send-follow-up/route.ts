...
async function internalSendWhatsapp(deviceId: string, target: string, message: string) {
    if (!deviceId || !target || !message) {
        const errorMsg = "WhatsApp Error: deviceId, target, or message is missing.";
        console.error(errorMsg, { deviceId: !!deviceId, target: !!target, message: !!message });
    ...
    try {
        ...
        if (!response.ok || responseJson.status === 'error') {
            const reason = responseJson.reason || 'Unknown error from WhaCenter';
            ...
            console.log(`Successfully queued follow-up message to ${target}`);
        }
    } catch (error) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!deviceId) {
             return NextResponse.json({ error: 'WhatsApp deviceId is not hardcoded on the server.' }, { status: 500 });
        }
